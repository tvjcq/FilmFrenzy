...
/**
 * Route pour obtenir des statistiques spécifiques à un film
 */
router.get("/movie/:id", async (req, res) => {
  try {
    ...
    console.error(
      `Error generating statistics for movie ${req.params.id}:`,
      error
    );
    res.status(500).json({
      status: "error",
      message: "Failed to generate movie statistics",
      error: error.message,
    });
  }
});
